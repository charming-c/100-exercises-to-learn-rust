    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(echo(first, Arc::clone(&fixed_reply)));
    let handle2 = tokio::spawn(echo(second, Arc::clone(&fixed_reply)));
    let (outcome1, outcome2) = tokio::join!(handle1, handle2);

}

pub async fn echo<T: Display + Send + Sync + 'static>(socket: TcpListener, fixed_reply: Arc<T>) -> Result<(), anyhow::Error> {
    loop {
        let (mut socket, _) = socket.accept().await?;

        let (_, mut writer) = socket.split();
        writer
        .write_all(format!("{}", fixed_reply).as_bytes())
        .await
        .unwrap();
    }
