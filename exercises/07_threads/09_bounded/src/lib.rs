...
impl TicketStoreClient {
    pub fn insert(&self, draft: TicketDraft) -> Result<TicketId, TrySendError<TicketDraft>> {
        ...
        match self.sender.try_send(Command::Insert { draft: draft.clone(), response_channel: sender }) {
    ...
    pub fn get(&self, id: TicketId) -> Result<Option<Ticket>, TrySendError<TicketId>> {
        let (sender, receiver) = sync_channel(1);
        match self.sender.try_send(Command::Get { id: id.clone(), response_channel: sender }) {
            Ok(_) => Ok(receiver.recv().unwrap()),
