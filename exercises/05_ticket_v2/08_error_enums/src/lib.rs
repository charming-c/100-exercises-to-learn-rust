...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match res {
        ...
        // Err(err) => {
        //     match err {
        //         TicketNewError::TitleError(string) => panic!("{}", string),
        //         TicketNewError::DescriptoinError(string) => Ticket::new(title.clone(), "Description not provided".into(), status.clone()).unwrap()
        //     }
        // }
        Err(TicketNewError::TitleError(str)) => {
            panic!("{}", str)
        },
        Err(TicketNewError::DescriptoinError(str)) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleError("Title cannot be longer than 50 bytes".into()));
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptoinError("Description cannot be empty".into()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptoinError("Description cannot be longer than 50 bytes".into()));
        }
